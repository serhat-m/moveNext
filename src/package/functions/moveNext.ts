...
function moveNext<TData extends { [key: string]: any }[], TSelector, TCurrentId extends undefined | TSelector>(opts: {
  data: TData
  direction: Direction
  ...
  const { data, direction, endBehaviour, selector, selectedId } = opts
  ...
  if (data.length > 0) {
    const indexOfFirstEntry = 0
    const indexOfLastEntry = data.length - 1
    let newIndex: false | number = false

    if (selectedId !== undefined) {
      for (const entry of data) {
        const selectorValue = selector(entry)

        if (typeof selectorValue !== typeof selectedId) {
          throw new TypeError(`type of selectedId (${typeof selectedId}) must be same as selector return value (${typeof selectorValue})`)
        }

        const entrySelectedValue = selectorValue === selectedId // Get current state
